...
CTile::CTile(CSprite* sprite, vector3d pos, CBox2D* bounding, int styleID)
{
	...
	m_Position						= vector3d(pos.x + 8, pos.y, 0.5);
...
{
	m_listSprite.at(m_State)->Render(CCamera::setPositionEntity(vector3d(m_Bounding->getX(), m_Bounding->getY(), 0.5f)), vector2d(1.0, 1.0), 0.0f, DRAWCENTER_MIDDLE_MIDDLE, true, FPS);
}
